...
const getNewsData = require("../NewsScraping/NewsScraper.js");
...
const football_json = require("../NewsFiles/football.json")
...
const getData = async (apiKey, request, res, user) => {
  ...
  data = await getNewsData(
    ...
    request.tbs,
    request.safe,
    request.google_domain,
    request.api_key
  );

  let html, news_results, sub_articles, current, nearNav, page_no, other_pages;
  if(expired == true)
  {
    console.log("andar")
    let data = {}
...
return response;
  ...
  for (let i = 0; i < page_no.length; i++) {
    let gl = request.gl;
    op[i] = `https://api.scrapingdog.io/news?api_key=${apiKey}&q=${request.q?.replace(/ /g, "+")}&gl=${gl ? gl : "us"}&page=${(page_no[i] - 1) * 10}`;
  }
  ...
  const scrapingdog_pagination = {
    ...
    if (!request.num) {
      ...
      user.requests = user.requests + 20*Math.ceil(news_results.length / 10);
      user.requestsLeft = user.quota - user.requests;
      user.apiCallsPerDay = user.apiCallsPerDay + 20*Math.ceil(news_results.length / 10);
    }
    ...
    data.status = 200
    if (sub_articles.length) {
      data.sub_articles = sub_articles;
    }
    if (news_results.length) {
